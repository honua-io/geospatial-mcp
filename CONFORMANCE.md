...
| **composition** | Additive. The **interactive composition tools** — currently `bind_interaction` and `remove_interaction`, which author the declarative `interactions` block of a composition document (see [`docs/adr/0030-declarative-interactions-and-layout.md`](docs/adr/0030-declarative-interactions-and-layout.md)), and `add_control` / `remove_control`, which author its `controls` collection of input affordances — the closed kind vocabulary that `control:{id}` references resolve against (see [`docs/adr/0031-composition-controls.md`](docs/adr/0031-composition-controls.md); no feature-editing draw control, per ADR-0028). Bindings and controls are data-only (closed event/verb/kind sets, `$event.*` substitution, no expression language) and mutate presentation wiring only — no source records, so `composition` is orthogonal to `mutation`. The wider interactive-composition family the reference ships under vendor names joins this profile through future ADRs. |
