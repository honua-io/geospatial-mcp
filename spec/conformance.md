...
|---|---|---|
| `emit_plan` | Tool fixtures that emit a canonical plan object (`plan_analysis` for Analyze; the plan-emitting equivalents for `PublishingPlan`, `BuilderPlan`, `DeploymentPlan` as named upstream) | Canonical plan object (`AnalysisPlan`, `PublishingPlan`, `BuilderPlan`, `DeploymentPlan`), expected `outputs` declaration per [planning.md §5.4](planning.md#54-boundary-crossing-fields); no validation verdict is asserted |
...
| `tool_result` | Tool fixtures that return a canonical resource object or handoff reference rather than a plan (`execute_plan` → `ExecutionJob` reference for Analyze, or the `PipelineService`-owned submission surface for Publish Data; `create_app_package` → `AppPackage`; `preview_app_package` → preview artifact; `create_map_package`, `refine_map_package`, `apply_style_preset`, `compose_mixed_protocol_map`, `preview_map_package` → `MapPackage` or preview artifact; `publish_result` → publication reference) | The returned canonical object by name and, where defined in [resources.md](resources.md), the `honua://` URI under which it is addressable; deferred shapes follow §2.4 |
...
| Build App | template binding, `create_app_package`, `preview_app_package`, package inspection | direct Builder tools → `AppPackage` (via `create_app_package`) or preview artifact (via `preview_app_package`); no `execute_plan` handoff per [planning.md §4.3](planning.md#43-build-app) |
