...
| `expected` | Exactly one of the four expected-behavior shapes defined in §2.3 |
...
|---|---|---|
| `emit_plan` | Tool fixtures for `plan_analysis`, `validate_plan`, builder planning | Canonical plan object (`AnalysisPlan`, `PublishingPlan`, `BuilderPlan`, `DeploymentPlan`), expected `outputs` declaration per [planning.md §5.4](planning.md#54-boundary-crossing-fields) |
| `emit_clarification` | Tool fixtures for `clarify_intent` and any planning tool when reason codes apply | `ClarificationRequest` with `reasonCodes[]` (subset of [planning.md §2.1](planning.md#21-trigger-conditions)) and per-question `kind` drawn from `ClarificationQuestionKind` |
...
or a response that mixes plan and clarification output. "Emit no plan and
no clarification" is not a legal expected shape (see
...
| `expectedTurns[]` | Ordered protocol turns: zero or more `clarify_intent` rounds, one `validate_plan`, one `execute_plan` or direct builder tool for Build App v1, and one result-package or `PublishedService` projection | [planning.md §5](planning.md#5-plan-handoff-semantics) |
...
| `expectedStepKinds[]` | Step kinds expected in the emitted plan, drawn from the per-family set in [planning.md §4](planning.md#4-per-family-planning-behavior) | planning.md |
...
| `expectedProvenanceFields[]` | `ProvenanceRecord` fields expected on the result projection, drawn from [resources.md §Result Package Resources](resources.md#result-package-resources) | resources.md |
...
| Publish Data | source inspection, schema and quality review, publish, refresh monitoring (deferred shape), deployment inspection | `execute_plan` → `PipelineService` execution → `PublishingResultPackage` with `PublishedService` (deferred shape) |
| Build App | template binding, `create_app_package`, `preview_app_package`, package inspection | direct Builder tools → `AppPackage` |
...
|---|---|---|---|---|
| 1. Clarification quality | Fires only when at least one in-scope reason code applies; each question uses one of the four canonical `ClarificationQuestionKind` values; `questionId` is unique across the intent lifecycle; rebinding is additive and idempotent | One question uses a kind outside the canonical four but no `questionId` collision occurs | A reason code outside `ClarificationReasonCode` is emitted; a `questionId` is reused; rebinding removes a previously resolved field | [planning.md §2.1, §2.2, §2.4, §2.6](planning.md#2-clarification-and-elicitation-semantics) |
| 2. AssumptionPolicy behavior | Non-suppressible reason codes always surface regardless of policy; suppressed reason codes bake resolved values into plan step `inputs` and the compact assumption audit names one string per suppressed reason code | The assumption audit is attached when no carrier field is available upstream (resolved values in step inputs remain the authoritative record per [planning.md §2.3](planning.md#23-assumptionpolicy-behavior)) | `DestructiveAction`, `PublishAction`, or `PolicyBoundary` is suppressed under any policy; a suppressed reason code lacks either the baked step input or the audit string | [planning.md §2.3](planning.md#23-assumptionpolicy-behavior) |
...
- **Conformance spec version per run.** The `specVersion` of this
  document that a run was evaluated against, for traceability during
  spec churn.
