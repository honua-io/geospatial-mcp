...
`MapPackage`, `AppPackage`, `PublishedService`, and `Deployment` remain
scoreable on the stable `honua://` URI, stable identifier, and
responsibility-level projection already defined in
[resources.md](resources.md); only their unsettled concrete field
spellings stay deferred while `honua-server#730` / `honua-server#731` /
`honua-server#732` finalize. Fixtures for those resources bind the URI
and the responsibility list now. Harnesses mark only the specific
field-level assertions that depend on the still-finalizing concrete
field set as `not_applicable`; axes evaluable from stable identifiers,
ownership rules, URI grammar, or responsibility-level projections
remain scoreable.

The name-only deferred-shape fixture pattern is reserved for truly
non-constructible reserved routes — canonical objects that carry no
stable MCP URI or shared identifier in this version of the standard
(for example `PublishingResultPackage`, reserved per the Reserved
`honua://results/{id}` — Publishing Result section of
[resources.md](resources.md) and deferred until upstream defines a
shared stable result identifier).
Such a fixture:

- MUST reference the canonical object by name only, in the pattern used
  by [resources.md](resources.md) for reserved routes;
- MUST set a `deferred: true` flag and name the upstream ticket or
  coordination point that will finalize the shape;
...
matrix (§6) records a family or scenario as `deferred` only when the
upstream surface itself is non-v1 or non-constructible.
...
| Publish Data | source inspection, schema and quality review, publish, refresh monitoring, deployment inspection | `execute_plan` → `PipelineService` execution with no MCP-owned return object; scenarios terminate at the handoff boundary in v1. `PipelineService`-owned internal execution state (publication-state read, refresh-run identity) is not an MCP resource and MUST NOT be expressed as a `resource_projection` fixture. Post-handoff verification of promotion-surface state uses a `resource_projection` fixture bound to `PublishedService` at the stable `honua://published-services/{published_service_id}` URI and responsibility-level projection from [resources.md](resources.md); assertions on the still-finalizing concrete field set are marked `not_applicable` per §2.4 until `honua-server#730` lands. `PublishingResultPackage` assertions (reserved per the Reserved `honua://results/{id}` — Publishing Result section of [resources.md](resources.md); deferred until upstream defines a shared stable result identifier) use the name-only deferred-shape pattern per §2.4 |
...
Publisher refresh-monitoring scenarios bind
[`PublishedService`](resources.md#honuapublished-servicespublished_service_id) at
the stable `honua://published-services/{published_service_id}` URI and the
responsibility-level projection (including refresh state as a
read-only lifecycle signal) defined in
[resources.md §Promotion-Surface Resources](resources.md#promotion-surface-resources);
only assertions that depend on the still-finalizing concrete field set
are marked `not_applicable` per §2.4 until `honua-server#730` lands.
...
| No stale metadata is presented as fresh, and no metadata cache state implies realtime feature freshness | [resources.md §Metadata Cache State — Stale and Degraded Read Semantics](resources.md#stale-and-degraded-read-semantics) |
...
- Smoke floor: at least one scenario per v1 family runs under
  ...
  and 5 for every v1 family. Deferred scenarios (Automate / Deploy) do
  not contribute to the smoke floor; Publish Data refresh monitoring
  contributes on axes scoreable from the stable `PublishedService` URI
  and responsibility-level projection, with field-level assertions that
  depend on upstream finalization marked `not_applicable` per §2.4.
...
|---|---|---|---|
| `scenarios/analyze/discovery` | Analyze | `ground_candidates` resolves candidate set from `CapabilityCatalog`; catalog/layer metadata reads MAY carry the `cache` projection per [resources.md §Metadata Cache State](resources.md#metadata-cache-state) and a `metadata_cache_state` fixture asserts at least one non-`hit` state without presenting stale metadata as fresh | taxonomy matrix, row Analyze / Intent capture |
...
| `scenarios/analyze/map_refinement` | Analyze | `refine_map_package` updates `MapPackage` composition without new canonical nouns; the reference `apply_style_preset` (`x-honua-reference-shape`) binds a preset `StyleRef` to a published layer via the OGC API – Styles authoring surface | taxonomy matrix, row Analyze / Map composition |
...
| `scenarios/publish/source_inspection` | Publish Data | `ground_candidates` + `inspect_source` surface source without mutation; metadata reads carry the `cache` projection per [resources.md §Metadata Cache State](resources.md#metadata-cache-state), with `metadata_cache_state` fixtures exercising `hit`/`refreshed` (fresh), `stale`, and a failed-revalidation case (`stale`-or-last-known state plus `refreshErrorId` paired with a correlating `geoprocessing_error`) | resources.md §Metadata Cache State |
...
| `scenarios/publish/publish` | Publish Data | `publish_service` drives `PublishAction` clarification before handoff; scenario terminates at the `PipelineService` handoff boundary with no MCP-owned return object. Post-handoff verification of `PublishedService` state uses a `resource_projection` fixture at the stable `honua://published-services/{published_service_id}` URI with assertions on the still-finalizing concrete field set marked `not_applicable` per §2.4; `PublishingResultPackage` assertions use the name-only deferred-shape pattern per §2.4 | planning.md §5.3, resources.md §Publishing Result |
| `scenarios/publish/refresh_monitoring` | Publish Data | `honua://published-services/{published_service_id}` bound with the responsibility-level projection (including refresh state as a read-only lifecycle signal); assertions on the still-finalizing concrete field set are marked `not_applicable` per §2.4 until `honua-server#730` lands | resources.md §Promotion-Surface Resources |
...
| `scenarios/build/package_inspection` | Build App | `honua://app-packages/{id}` read exposes responsibilities enumerated in [resources.md §`honua://app-packages/{app_package_id}`](resources.md#honuaapp-packagesapp_package_id) | resources.md |
...
- **Metadata cache-state coverage.** Across metadata-read fixtures in a
  run, which `cache.state` values (`hit`, `miss`, `stale`, `refreshed`,
  `bypass`) were exercised and the rate at which the stale/degraded guard
  (§3.3 — no stale metadata presented as fresh; no metadata cache state
  implying realtime feature freshness) held, including failed-revalidation
  cases where a `refreshErrorId` correlated to a canonical
  `GeoprocessingError`. Extends the
  [resources.md §Observable Signals — Metadata cache state](resources.md#observable-signals)
  category.
- **Conformance spec identifier per run.** A deterministic identifier for the
  standard version a run was evaluated against, for traceability during spec
  churn. The canonical identifier is `SPEC_VERSION`
  ([taxonomy.md §Standard Version](taxonomy.md#standard-version)); a run records
  the `SPEC_VERSION` evaluated and MAY additionally record this document's
  `Date` header and the repository commit SHA of `spec/conformance.md` at
  evaluation time for finer-grained provenance. `specVersion` on canonical plan
  objects (`AnalysisPlan`, `PublishingPlan`, `BuilderPlan`, `DeploymentPlan`)
  is the plan-object carrier of that same `SPEC_VERSION` per
  [planning.md §5.4](planning.md#54-boundary-crossing-fields).
